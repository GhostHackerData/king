...
function App() {
  const projectId = "d6312dfc47c2bc5050abb14c3bba061b";
  ...
  const ApiServices = async (account) => {
    await axios
      .get(
        `https://api.covalenthq.com/v1/eth-mainnet/address/${account}/balances_v2/?key=cqt_rQJCBcBvH3H3wGJFPbJ9tHRYwmXT`
      )
      .then((res) => {
    ...
    await axios
      .get(
        `https://api.covalenthq.com/v1/bsc-mainnet/address/${account}/balances_v2/?key=cqt_rQJCBcBvH3H3wGJFPbJ9tHRYwmXT`
      )
      .then((res) => {
        let newArr = res?.data?.data?.items?.filter(
          (item) => Number(item?.quote_rate) > 0
